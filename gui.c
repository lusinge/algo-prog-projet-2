...
static void activate_cb(GtkApplication *app, gpointer user_data)
{
	...
	gtk_window_set_titlebar(GTK_WINDOW(data->window), header_bar);
	...
	g_signal_connect(entry, "search-changed", G_CALLBACK(on_search_changed), data);
	...
	gtk_search_bar_set_key_capture_widget(GTK_SEARCH_BAR(search_bar), data->window);
	...
	g_signal_connect_swapped(plus_button, "clicked", G_CALLBACK(show_add_word_dialog), data);
